...
function main_looper(mytree)
    for evt in mytree
        ...
        pass_initial_cut = begin
            ...
            v_Z_pair, v_Z_wgt = WVZAnalysis.Find_Z_Pairs(v_l_pid, v_l_tlv, v_l_wgt)
            length(v_Z_wgt) == 0 && return false

            best_p = first(v_Z_pair)
            abs(mass(v_l_tlv[best_p[1]] + v_l_tlv[best_p[2]]) - Z_m) > 20 * 10^3 && return false
